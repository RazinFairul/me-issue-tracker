...
serve(async (req: Request) => {
  ...
  try {
    ...
    for (const issue of issues || []) {
      ...
      if (daysDiff === 3) {
        shouldSend = true;
        // Subjek bersih dan profesional tanpa perkataan 'spammy' atau emoji
        subjectPrefix = "Due Soon Notification";
        titleHeader = "Due Date Reminder";
        headerColor = "#d97706";
        btnColor = "#2563eb";
        descriptionText = `The following issue assigned to you is approaching its target closing date in <strong>3 days</strong>:`;
      } else if (daysDiff === -3) {
        shouldSend = true;
        subjectPrefix = "Action Required: Overdue Notice (3 Days)";
        ...
        shouldSend = true;
        subjectPrefix = "Urgent: Overdue Notice (6 Days)";
        titleHeader = "Critical Overdue Notice";
        headerColor = "#7f1d1d";
        btnColor = "#7f1d1d";
        descriptionText = `The following issue is <strong>overdue by 6 days</strong> and requires prompt resolution:`;
      }
      ...
      if (shouldSend) {
        ...
        const issueName = issue.what_issue || "Pending Issue";
        const reporterName = issue.staff_name || issue.staff_id || "-";
        const subject = `${subjectPrefix} - Issue: ${issueName}`;
        ...
        const htmlContent = '<div style="font-family: Arial, sans-serif; padding: 24px; color: #333333; max-width: 600px; border: 1px solid #e2e8f0; border-radius: 10px; background-color: #ffffff;">' +
          ...
          '<table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 14px;">' +
            `<tr><td style="padding: 10px; font-weight: bold; background-color: #f8fafc; border: 1px solid #e2e8f0; width: 35%;">Issue:</td><td style="padding: 10px; background-color: #f8fafc; border: 1px solid #e2e8f0;">${issueName}</td></tr>` +
          ...
          '<p style="font-size: 14px; line-height: 1.5; margin-bottom: 20px;">' +
            'Please update the issue status in the ME Data Tracker system once the issue has been addressed.' +
          '</p>' +
          ...
          '</div>' +
          `<p style="font-size: 13px; color: #475569; text-align: center; margin-top: 10px;">Link: <a href="${APP_URL}" style="color: #2563eb;">${APP_URL}</a></p>` +
